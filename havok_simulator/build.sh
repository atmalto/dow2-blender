...
qmake.exe ../havok_simulator.pro -spec win32-msvc2008 "${QMAKE_CONFIG[@]}"
nmake.exe clean || true
nmake.exe "$NMAKE_TARGET"

mkdir -p "$OUTPUT_DIR"
rm -f "$OUTPUT_DIR"/QtCore4.dll "$OUTPUT_DIR"/QtGui4.dll "$OUTPUT_DIR"/QtOpenGL4.dll \
	  "$OUTPUT_DIR"/QtCored4.dll "$OUTPUT_DIR"/QtGuid4.dll "$OUTPUT_DIR"/QtOpenGLd4.dll
